...
<div style="width:95%; padding:2px; height:190px; margin-top:2px; padding:5px 10px 5px 10px; border:#0C3 dashed 3px; position:relative;">
    <span class="t botli">
        <div style="text-align:start">最新消息區</div>
        <?php
        $total = $News->count(['sh' => 1]);
        if ($total > 5) {
        ?>
            <div style="text-align:end"><a href="?do=news">more</a></div>
        <?php
        }
        ?>
    </span>
    ...
    <ul class="ssaa" style="list-style-type:decimal;font-size:14px">
        <?php
